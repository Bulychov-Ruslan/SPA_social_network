...
def handle_request(request, pk, status):
    ...
    # Проверка, что запрос существует и имеет статус 'sent'
    if not friendship_request or friendship_request.status != FriendshipRequest.SENT:
        return JsonResponse({'message': 'Неверный запрос на дружбу'})
    ...
    if status == FriendshipRequest.ACCEPTED:
        ...
        # Создание уведомления о принятии запроса на дружбу
        ...
        # Создание уведомления о отклонении запроса на дружбу
